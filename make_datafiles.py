...
def get_art_abs(story_file):
    ...
    # Make article into a single string, putting <s> and </s> tags around the
    # sentences
    article = ' '.join(
        ["%s %s %s" % (SENTENCE_START, sent, SENTENCE_END) for sent in article_lines])

    # Make abstract into a single string, putting <s> and </s> tags around the
    # sentences
    abstract = ' '.join(
...
def write_to_bin(url_file, out_file, makevocab=False):
    ...
    with open(out_file + '.src', 'wb') as src_writer:
        with open(out_file + '.tgt', 'wb') as tgt_writer:
            for idx, s in enumerate(story_fnames):
                ...
                src_writer.write(article + '\n')
                tgt_writer.write(abstract + '\n')
    ...
    print "Finished writing files %s and %s" % (out_file + '.src', out_file + '.tgt')
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print "USAGE: python make_datafiles.py <cnn_stories_dir> <dailymail_stories_dir> <processed_dir>"
        sys.exit()
    ...
    cnn_tokenized_stories_dir = processed_dir + '/' + cnn_tokenized_stories_dir
    dm_tokenized_stories_dir = processed_dir + '/' + dm_tokenized_stories_dir
